...
int main() {
  ...
  CHECK_MSG(cgraph_ivec_equal(g->from, (CGRAPH_INTEGER []){0, 2, 1, 0, 1, 0, 2, 5}, 8), "g->from");
  CHECK_MSG(cgraph_ivec_equal(g->to, (CGRAPH_INTEGER [])  {1, 3, 3, 2, 2, 3, 5, 3}, 8), "g->to");
  CHECK_MSG(cgraph_ivec_equal(g->oi, (CGRAPH_INTEGER [])  {0, 3, 5, 4, 2, 1, 6, 7}, 8), "g->oi");
  CHECK_MSG(cgraph_ivec_equal(g->ii, (CGRAPH_INTEGER [])  {0, 3, 4, 5, 2, 1, 7, 6}, 8), "g->ii");
  ...
  CHECK_MSG(cgraph_ivec_equal(g->from, (CGRAPH_INTEGER []){2, 0, 0, 2, 5}, 5), "g->from 1");
  CHECK_MSG(cgraph_ivec_equal(g->to, (CGRAPH_INTEGER [])  {3, 2, 3, 5, 3}, 5), "g->to 1");
  CHECK_MSG(cgraph_ivec_equal(g->oi, (CGRAPH_INTEGER [])  {1, 2, 0, 3, 4}, 5), "g->oi 1");
  CHECK_MSG(cgraph_ivec_equal(g->ii, (CGRAPH_INTEGER [])  {1, 2, 0, 4, 3}, 5), "g->ii 1");
  ...
  CHECK_MSG(cgraph_ivec_equal(g->from, (CGRAPH_INTEGER []){2, 0, 5}, 3), "g->from");
  CHECK_MSG(cgraph_ivec_equal(g->to, (CGRAPH_INTEGER [])  {3, 3, 3}, 3), "g->to");
  CHECK_MSG(cgraph_ivec_equal(g->oi, (CGRAPH_INTEGER [])  {1, 0, 2}, 3), "g->oi");
